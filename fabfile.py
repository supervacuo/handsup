...
env.project_name = "handsup"
env.virtual_env = "/usr/local/virtualenvs/handsup"
env.tmp_dir = mkdtemp()
...
def manage(args):
    ...
    with virtualenv():
        sudo('SECRET_KEY="notsosecret" django-admin.py %s --settings=handsup.settings.%s --pythonpath=%s' % (
...
def deploy(release_name=None):
    ...
    with cd(release_root), virtualenv():
        sudo('pip install -q -r requirements/production.txt')
